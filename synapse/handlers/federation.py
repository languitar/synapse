...
class FederationHandler(BaseHandler):
                ...
                try:
                    ...
                    for p in prevs - seen:
                        ...
                        with nested_logging_context(p):
                            ...
                            remote_state = await self._get_state_at_missing_prev_event(
                                origin, room_id, p
                            )
    ...
    async def _get_state_at_missing_prev_event(
        self,
        destination: str,
        room_id: str,
        event_id: str,
        ...
        missing_events = desired_events - fetched_events.keys()

        if missing_events:
            # if there are missing events, we may need the auth events too. Let's
            # check which we already have.
            # TODO: we probably won't actually need all of the auth events, since we
            #   already have a bunch of the state events. It would be nice if the
            #   federation api gave us a way of finding out which we actually need.
            logger.debug(
                "we are missing %i events: checking for missing auth events",
                len(missing_events),
            )
            desired_auth_events = set(auth_event_ids) - fetched_events.keys()
            have_auth_events = await self.store.have_seen_events(desired_auth_events)
            missing_auth_events = desired_auth_events - have_auth_events
            logger.debug("we are missing %i auth events", len(missing_auth_events))

            missing_events |= missing_auth_events
            logger.debug(
                "Fetching %i events from remote",
                len(missing_events),
            )
            await self._get_events_and_persist(
                destination=destination, room_id=room_id, events=missing_events
            ...
            logger.debug("We already have all %i state events", len(desired_events))
